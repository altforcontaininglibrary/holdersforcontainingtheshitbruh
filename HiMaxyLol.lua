			...
			if holding then
				...
				if them then
					...
					if them then
						...
						local goal = root.CFrame + (humanoid.MoveDirection*prediction) + (root.Velocity*prediction/10)
						...
						if UIS.MouseBehavior ~= Enum.MouseBehavior.LockCurrentPosition then
							if UIS.MouseBehavior == Enum.MouseBehavior.LockCenter or not onScreen then					
								Camera.CFrame = Camera.CFrame:Lerp(CFrame.new(Camera.Focus.Position,goal.Position),smoothing)
							else
								mousemoveabs(point.X,point.Y+46)
							end
						end
